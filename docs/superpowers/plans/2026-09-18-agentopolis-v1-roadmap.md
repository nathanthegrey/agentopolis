...
| 7 | deploy | 13 (supervision), 15, 16 | systemd unit with notify/watchdog/`KillMode=mixed`, `EnvironmentFile`, the nightly backup cron, `runs/` rotation, `/healthz` and `/diag`, install doc for the owner (manifest paste, two tokens, first hire), live check 6 on the VPS | every line of spec section 15 holds on the VPS |
